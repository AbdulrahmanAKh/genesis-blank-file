...
export const GroupEventsPreview: React.FC<GroupEventsPreviewProps> = ({ groupId }) => {
  ...
  const loadEvents = async () => {
    try {
      ...
      // Check group ownership and admin status
      ...
      // Check if current user is the group owner or admin
      if (user && groupData) {
        const isGroupOwner = groupData.created_by === user.id;
        
        // Also check if user is an admin in the group
        ...
        const isGroupAdmin = memberData?.role === 'admin' || memberData?.role === 'owner';
        setIsOwner(isGroupOwner || isGroupAdmin);
      }
