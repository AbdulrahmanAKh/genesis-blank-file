...
const HeroSection = () => {
  ...
  const { data: eventsData, isLoading } = useSupabaseQuery({
    queryKey: ['trending-events'],
    queryFn: () => eventsService.getAll()
  });

  const trendingEvents = eventsData?.data?.filter(event => event.featured === true)?.slice(0, 3)?.map(event => ({
    id: event.id,
    title: event.title_ar || event.title,
    city: event.location_ar || event.location,
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {isLoading ? (
            ...
            <div className="col-span-full text-center py-12">
              <h3 className="text-lg font-medium mb-2">لا توجد فعاليات رائجة حالياً</h3>
              <p className="text-muted-foreground">ستتم إضافة فعاليات رائجة قريباً</p>
            </div>
          ) : (
