...
const CheckoutPage = () => {
  ...
  // Mock data
  const userPoints = 85;
  const pointsValue = Math.min(Math.floor(userPoints * 0.1), 20); // Max 20 SAR or 10% of price
  const basePrice = eventPrice || 150;
  const vatRate = 0.15;
  ...
  // Mock event data
  const eventData = {
    title: eventTitle || "هايكنج جبل طويق المتقدم",
    date: "2024-03-15",
    time: "06:00",
    location: "الرياض - جبل طويق", 
    ...
    cancellationPolicy: "إلغاء مجاني حتى 48 ساعة قبل الموعد"
  };

  // Available coupons (mock data)
  const availableCoupons = [
    { code: 'SAVE10', type: 'percentage', value: 10, description: 'خصم 10%' },
    { code: 'FLAT50', type: 'fixed', value: 50, description: 'خصم 50 ريال' },
    { code: 'WELCOME', type: 'percentage', value: 15, description: 'خصم 15% للعضوية الجديدة' }
  ];
  ...
  const applyCoupon = () => {
    ...
    if (coupon) {
      ...
      toast({
        ...
        description: `تم تطبيق كوبون ${coupon.code} - ${coupon.description}`
      });
      setCouponCode('');
    } else {
      toast({
